---
title: LockdownNativeMethods.lockdownd_start_session method (1 of 2)
---

Opens a session with lockdownd and switches to SSL mode if device wants it.

```csharp
public static LockdownError lockdownd_start_session(LockdownClientHandle client, string hostId, 
    out IntPtr sessionId, ref int sslEnabled)
```

| parameter | description |
| --- | --- |
| client | The lockdownd client |
| host_id | The HostID of the computer |
| session_id | The new session_id of the created session |
| ssl_enabled | Whether SSL communication is used in the session |

## Return Value

LOCKDOWN_E_SUCCESS on success, LOCKDOWN_E_INVALID_ARG when a client or host_id is NULL, LOCKDOWN_E_PLIST_ERROR if the response plist had errors, LOCKDOWN_E_INVALID_HOST_ID if the device does not know the supplied HostID, LOCKDOWN_E_SSL_ERROR if enabling SSL communication failed

## See Also

* enum [LockdownError](../LockdownError.md)
* class [LockdownClientHandle](../LockdownClientHandle.md)
* class [LockdownNativeMethods](../LockdownNativeMethods.md)
* namespace [iMobileDevice.Lockdown](../../iMobileDevice-net.md)

---

---
title: LockdownNativeMethods.lockdownd_start_session method (2 of 2)
---

```csharp
public static LockdownError lockdownd_start_session(LockdownClientHandle client, string hostId, 
    out string sessionId, ref int sslEnabled)
```

## See Also

* enum [LockdownError](../LockdownError.md)
* class [LockdownClientHandle](../LockdownClientHandle.md)
* class [LockdownNativeMethods](../LockdownNativeMethods.md)
* namespace [iMobileDevice.Lockdown](../../iMobileDevice-net.md)

<!-- DO NOT EDIT: generated by xmldocmd for iMobileDevice-net.dll -->
