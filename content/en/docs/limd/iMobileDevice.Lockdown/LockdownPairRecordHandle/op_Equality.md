---
title: LockdownPairRecordHandle Equality operator
---

Determines whether two specified instances of [`LockdownPairRecordHandle`](../LockdownPairRecordHandle.md) are equal.

```csharp
public static bool operator ==(LockdownPairRecordHandle value1, LockdownPairRecordHandle value2)
```

| parameter | description |
| --- | --- |
| value1 | The first pointer or handle to compare. |
| value2 | The second pointer or handle to compare. |

## Return Value

`true` if *value1* equals *value2*; otherwise, `false`.

## See Also

* class [LockdownPairRecordHandle](../LockdownPairRecordHandle.md)
* namespace [iMobileDevice.Lockdown](../../iMobileDevice-net.md)

<!-- DO NOT EDIT: generated by xmldocmd for iMobileDevice-net.dll -->
